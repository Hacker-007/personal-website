...
// Transformer to remove italic font-style from Shiki output
const removeItalicsTransformer: ShikiTransformer = {
  span(node) {
    if (node.properties.style && typeof node.properties.style === 'string') {
      node.properties.style = node.properties.style.replace(/font-style:\s*italic;?/g, '')
    }
  },
}
...
export function EnviousEditor() {
  ...
  // Highlight code with Shiki whenever it changes
  useEffect(() => {
    codeToHtml(code, {
      lang: 'rust',
  ...
  return (
    <div className="border-border flex h-80 w-full flex-col overflow-hidden rounded-xs border font-mono">
      {/* Editor Header */}
      <div className="border-border flex shrink-0 items-center border-b bg-[#0e0e14] px-3 py-2.5">
        <span className="text-muted flex-1 text-left text-sm">main.envy</span>
      </div>
      ...
      <div className="bg-surface-1 relative flex min-h-0 flex-1">
        ...
        <div className="relative min-h-0 flex-1">
          ...
          <div className="pointer-events-none absolute inset-0 overflow-hidden p-2 pl-3">
            <div
              className="font-mono text-sm leading-5 [&_pre]:m-0 [&_pre]:bg-transparent! [&_pre]:p-0 [&_code]:font-[inherit]"
